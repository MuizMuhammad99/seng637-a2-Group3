...
Input domains are divided into ranges based on the behavior that the code should exhibit.
Each partition is treated as an equivalent data set.
At least one test case will be derived from each partition.
2. Boundary Value Analysis
Test cases are designed around the values at the edge of equivalence partitions.
Both valid and invalid boundary values will be tested.
3. Error Guessing
Based on experience, testers will derive test cases that might uncover errors in boundary conditions or other non-obvious paths.
### Test Environment
